...
public class MealsUtil {
    ...
    public static List<MealTo> mealsTo = Arrays.asList(
            createWithExcess(meals.get(0), false),
            createWithExcess(meals.get(1), true),
            createWithExcess(meals.get(2), false),
            createWithExcess(meals.get(3), true),
            createWithExcess(meals.get(4), false),
            createWithExcess(meals.get(5), false)
    );
...
//                      Collectors.toMap(Meal::getDate, Meal::getCalories, Integer::sum)
    ...
    public static MealTo createWithExcess700(Meal meal) {
        return new MealTo(meal.getId(), meal.getDateTime(), meal.getDescription(), meal.getCalories(), meal.getCalories() >= 700);
    }

    public static List<MealTo> convertToMealsTo(Collection<Meal> meals){
        return meals.stream().
                map(e -> createWithExcess(e, e.getCalories() >= 700)).
                collect(Collectors.toList());
    }
}